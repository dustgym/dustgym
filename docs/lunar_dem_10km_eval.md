...
| **Neukum coefficient vector** (from MintonGroup/cratermaker) | **Verify first** | The numeric coefficients are published scientific facts (not copyrightable) — cite Neukum/Ivanov/Hartmann 2001 by author/year. But verify cratermaker's LICENSE (MIT/BSD) before copying the vector verbatim into CC0 code. |
...
|---|---|---|---|
| **Crater production** | Neukum polynomial `log₁₀ N_cum`; surface age committed **T = 3.5 Gyr**; valid 10 m–1000 km, **do not extrapolate below ~10 m** | Ivanov/Neukum/Hartmann 2001 (Space Sci. Rev. 96:55). **Coeff vector transcribed from MintonGroup/cratermaker; primary table not directly verified** (linear coeff 8.25e-4 here vs paper 8.38e-4 — reconcile before relying) | `[CALIB]` |
...
| **Absolute roughness anchor** | read per-pixel from **Product 90 `LDRM_RMSD`** at the 100 m baseline; seed fbm variance so synthesized roughness-vs-baseline matches | Product 90 (Barker 2023) — **per-tile DATA, auditable** | data |
